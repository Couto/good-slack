...
module.exports = internals.GoodSlack = function(events, config) {
  ...
  Hoek.assert(typeof config.channel === 'string', 'config.channel must be a string');
  Hoek.assert(typeof config.url === 'string', 'config.url must be a string');

  var settings = Hoek.applyToDefaults(internals.defaults, config.options || {});
...
internals.GoodSlack.prototype.init = function(attachment) {};
